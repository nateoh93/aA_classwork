...
def harrison_ford
  ...
  # Actor.joins(:movies).select(:id, :title).where("name = (?)", "Harrison Ford").where.not("ord = (?)", 1)
end
...
def biggest_cast
  ...
  Movie
    ...
    # Movie.joins(:actors).select("movies.id", :title).group("movies.id").order("COUNT(actors.id)").limit(3)
end
...
def directed_by_one_of(them)
  ...
  # Movie.select(:id, :title).where(director_id: them)
end
